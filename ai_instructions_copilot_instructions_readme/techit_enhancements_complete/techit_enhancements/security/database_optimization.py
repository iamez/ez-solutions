...
class Command(BaseCommand):
    ...
    def create_indexes(self):
        ...
        indexes = [
            ...
            # Services indexes
            (
                "CREATE INDEX IF NOT EXISTS idx_services_user ON services_service(user_id);",
                "Service user",
            ),
            (
                "CREATE INDEX IF NOT EXISTS idx_services_status ON services_service(status);",
                "Service status",
            ),
            (
                "CREATE INDEX IF NOT EXISTS idx_services_expiry ON services_service(expiry_date) WHERE status = 'active';",
                "Service expiry",
            ),
            (
                "CREATE INDEX IF NOT EXISTS idx_services_created ON services_service(created_at DESC);",
                "Service created",
            ),
            # Orders indexes
            ...
            (
                "CREATE INDEX IF NOT EXISTS idx_orders_created ON orders_order(created_at DESC);",
                "Order created",
            ),
            # Tickets indexes
            (
                "CREATE INDEX IF NOT EXISTS idx_tickets_user ON tickets_ticket(user_id);",
                "Ticket user",
            ),
            (
            ...
            (
                "CREATE INDEX IF NOT EXISTS idx_tickets_priority ON tickets_ticket(priority);",
                "Ticket priority",
            ),
            # Composite indexes
            (
                "CREATE INDEX IF NOT EXISTS idx_services_user_status ON services_service(user_id, status);",
                "Service user+status",
            ),
            (
        ...
        with connection.cursor() as cursor:
            for sql, description in indexes:
                try:
                    ...
                    self.stdout.write(
                        self.style.WARNING(f"⚠ Failed to create index ({description}): {str(e)}")
                    )
