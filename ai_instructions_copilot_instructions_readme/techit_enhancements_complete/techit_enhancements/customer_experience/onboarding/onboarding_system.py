...
class OnboardingStep(models.Model):
    ...
    email_delay_hours = models.IntegerField(
        default=0, help_text="Send email X hours after previous step"
    )

    # In-app tour
    tour_element_id = models.CharField(
        max_length=100, blank=True, help_text="DOM element ID for tour highlight"
    )
...
class UserOnboarding(models.Model):
    ...
    def skip_step(self, step):
        ...
        UserOnboardingProgress.objects.update_or_create(
            user_onboarding=self,
            step=step,
            defaults={"status": "skipped", "completed_at": timezone.now()},
        )
    ...
    def move_to_next_step(self):
        ...
        next_step = OnboardingStep.objects.filter(
            is_active=True, order__gt=self.current_step.order
        ).first()
        ...
        if next_step:
            ...
            if next_step.send_email:
                ...
                send_onboarding_email.apply_async(
                    args=[self.user.id, next_step.id], countdown=next_step.email_delay_hours * 3600
                )
        else:
    ...
    def get_incomplete_steps(self):
        ...
        completed_step_ids = self.progress.filter(status__in=["completed", "skipped"]).values_list(
            "step_id", flat=True
        )

        return OnboardingStep.objects.filter(is_active=True).exclude(id__in=completed_step_ids)
...
class UserOnboardingProgress(models.Model):
    ...
    user_onboarding = models.ForeignKey(
        UserOnboarding, on_delete=models.CASCADE, related_name="progress"
    )
...
class OnboardingManager:
    ...
    def check_step_completion(user, step_slug):
        ...
        try:
            ...
            progress = UserOnboardingProgress.objects.get(user_onboarding=onboarding, step=step)
            return progress.status == "completed"
        except (
            UserOnboarding.DoesNotExist,
            OnboardingStep.DoesNotExist,
            UserOnboardingProgress.DoesNotExist,
        ):
            return False
            ...
            for step in steps:
                try:
                    progress = UserOnboardingProgress.objects.get(
                        user_onboarding=onboarding, step=step
                    )
                ...
                checklist.append(
                    {"step": step, "status": status, "is_current": step == onboarding.current_step}
                )
